...
local draw_crit = {
    target_offset = { .0, .0 },
    only_in_alt_mode = true, -- Corregí el typo en 'nly_in_alt_mode'
    scale = 1.125,
    scale_with_zoom = true,
    alignment = "center"
}
...
local function turret_mags(t)
    ...
    if x == nil then
        if t and t.valid then
            x = t.get_inventory(defines.inventory.turret_ammo).get_bar()
            if x == nil then
                x = 0
            end
        else
    ...
    if disp_mode == "shot" then
        ...
        for j = 1, #i do
            ...
            if s.valid_for_read then
                m = m + s.count
                shots = shots + mag_size(s.name) * (s.count - 1) + s.ammo
            end
        end
        if m == 0 then return
        elseif m <= 1 then
            info = draw_crit
        elseif m < turret_mags(t.name) then
            info = draw_low
        else
            info = draw_norm
        end
...
script.on_event({defines.events.on_force_friends_changed, defines.events.on_forces_merged}, team_views)
script.on_load(set)
script.on_init(set)